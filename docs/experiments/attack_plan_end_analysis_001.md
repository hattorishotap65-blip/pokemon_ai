...
logged top_candidates 内に attack candidate (type=13) が存在するケースは **ゼロ** だった。

少なくとも logged top_candidates の範囲では、End と同時に attack candidate は確認されなかった。
ただし top_candidates は診断用に絞られた候補のため、legal options 全体に attack option が
存在したかは、この診断だけでは断定できない。
...
1. **policy 側の End 抑制は慎重に**: top_candidates に attack がない場合の End 抑制は illegal action のリスク
2. **legal options 全体の確認**: top_candidates だけでなく all legal actions / candidate count の
   追加ログが必要（top_candidates は絞り込み後の候補のため）
3. **attack_plan の plan 生成精度を改善**: エネルギー条件を考慮して
   「実際に攻撃可能か」を判定に加えると missed_ko_plan が減る
4. エネルギー付きの attack readiness check を plan 生成に組み込む候補
5. diagnostics の attack decision フィルタ（type=13 の decision のみ集計）を次 PR で検討
