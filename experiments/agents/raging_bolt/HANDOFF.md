...
最終更新: 2026-07-23 / 最終コミット: `e6a3f8a`
...
| 対戦相手 | 勝率 (100戦) | 備考 |
|----------|:---:|------|
| top_lucario_1084 | **20.0%** | 最強ベンチマーク。セッション開始時6.5% |
| dragapult (sandbox) | **25.3%** | ベンチダメージ評価導入前は10% |
| megastarmie (sandbox) | **55%** | 導入前41% |

（2026-07-08時点の17〜18%から、第9章の相手エネルギー推定+終盤深読みで20.0%まで到達。3章のリッジ回帰による重み学習とUCB1バンディット配分は検証したが不採用/保留 — 詳細は下記セクション参照）
...
| AIMA第5章: UCB1バンディット配分(`rule_ucb1_search`, 既定OFF) | ルカリオ16%/ミラー53-47、共に有意差なし | 探索定数C=200・予算とも未較正。構造的欠陥ではなく較正不足の可能性 — 再挑戦の価値あり(下記) |
...
1. **UCB1の較正** — `ucb1_exploration_c`(既定200)と予算(`ucb1_extra_rollouts`=3, `ucb1_endgame_extra_rollouts`=8)を`auto_tune.py`スタイルでスイープ。100gで有意差なしだった原因が較正不足なら改善の余地あり
